...
class AtomicRules(StandardRules):
    ...
    def apply_move(self, move: Move) -> GameState:
        ...
        is_en_passant = isinstance(moving_piece, Pawn) and move.end == self.state.ep_square
        ...
        new_state = GameState(
            ...
            castling_rights=self._update_castling_rights_after_explosion(new_board),
    ...
    def validate_move(self, move: Move) -> MoveLegalityReason:
        ...
        if isinstance(piece, King):
            if self.state.board.get_piece(move.end) or move.end == self.state.ep_square:
                return self.MoveLegalityReason.OWN_PIECE_CAPTURE # Or a better reason? "KINGS_CANT_CAPTURE"
        ...
        if self._kings_adjacent(next_state.board):
            return self.MoveLegalityReason.KING_EXPLODED # Or "KINGS_ADJACENT"
